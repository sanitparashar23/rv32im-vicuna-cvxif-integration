// vproc_top.sv (renamed here: vproc_top_unused.sv) -- REFERENCE ONLY, NOT USED
//
// This is Vicuna's own stock top-level integration wrapper around vproc_core,
// included here for reference and comparison purposes only. It is NOT part of
// this project's build (not listed in filelist.f, not instantiated anywhere
// in rtl/integration/).
//
// Vicuna officially targets Ibex and CV32E40X as its reference host cores.
// vproc_top.sv is built around those cores' cache-parameterized memory
// interface conventions (ICACHE_SZ, DCACHE_SZ, etc.), which do not match
// the Jeffrey Core RV32IM scalar pipeline used in this project.
//
// rtl/integration/vicuna_wrapper.sv is the original replacement built
// specifically for this integration: it serves the same architectural role
// (bridging vproc_core to a main scalar core over CV-X-IF) but is written
// from scratch against Jeffrey Core's actual memory and pipeline interface,
// rather than adapting this file.
//
// Original source: https://github.com/vproc/vicuna
// License: Solderpad Hardware License v2.1 / Apache License 2.0
//
// Author note: Sanit Parashar
